...
def test_crud_geo_import_create(db_with_meta):
    ...
    assert (
        ...
        .filter(models.User.user_id == geo_import.created_by)
        .first()
        is not None
    )
    assert (
...
def test_crud_geo_import_get(db_with_meta):
    ...
    assert (
        crud.geo_import.get(db=db, uuid=geo_import.uuid).namespace_id
        == geo_import.namespace_id
    )
    assert (
        crud.geo_import.get(db=db, uuid=geo_import.uuid).meta_id == geo_import.meta_id
    )
    assert (
        crud.geo_import.get(db=db, uuid=geo_import.uuid).created_by
        == geo_import.created_by
    )
    assert (
        crud.geo_import.get(db=db, uuid=geo_import.uuid).created_at
        == geo_import.created_at
    )
