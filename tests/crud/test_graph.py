...
def test_crud_graph_create(db_with_meta):
    ...
    assert (set((edge.geo_id_1, edge.geo_id_2) for edge in created_graph.edges) == {(geo[0][0].geo_id, geo[1][0].geo_id)}
        or set((edge.geo_id_1, edge.geo_id_2) for edge in created_graph.edges) == {(geo[1][0].geo_id, geo[0][0].geo_id)})
